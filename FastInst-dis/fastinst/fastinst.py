...
class FastInst(nn.Module):
    ...
    def from_config(cls, cfg):
        ...
        d_weight_dict = {"loss_kd": 0.02, "loss_mask_dis": 0.05, "loss_dice_dis": 0.05}
        dis_weight_dict = {}
        for i in range(3):
            dis_weight_dict.update({k + f"_{i}": v for k, v in d_weight_dict.items()})
        weight_dict.update(dis_weight_dict)
    ...
    def forward(self, batched_inputs):
        ...
        if self.training:
            ...
            mask_pred_results = outputs["pred_masks"]  # (1, 100, 32, 108)
