    ...
    def forward(self, x, mask_features, targets=None):
        if self.training:
            ...
            pixel_pos_embeds = F.interpolate(self.meta_pos_embed, size=pixel_feature_size,
                                             mode="bilinear", align_corners=False)  # (1, c, pixel_h, pixel_w)
            proposal_pos_embeds = F.interpolate(self.meta_pos_embed, size=proposal_size,
                                                mode="bilinear", align_corners=False)  # (1, c, proposal_h, proposal_w)

            pixel_features = x[2].flatten(2).permute(2, 0, 1)  # (pixel_h*pixel_w, bs, c)
            pixel_pos_embeds = pixel_pos_embeds.flatten(2).permute(2, 0, 1)  # (pixel_h*pixel_w, bs, c)
            ...
            if self.num_aux_queries > 0:
                ...
                query_pos_embeds = torch.cat([query_pos_embeds, aux_query_pos_embed], dim=0)  # (108, bs, c)
            ...
            for i in range(self.num_layers):
                ...
                query_features = query_features_reserve.copy()[start_q:end_q]  # q0;q0 q0-1;q0-1 q0-2 q0-1-2;
                pixel_features = pixel_feature_memory.copy()[-1]  # p0;p0-1;p0-1-2;
                # SQR
                ...
                query_pos_embeds_qr = query_pos_embeds.repeat(1, fakesetsize, 1)  # (108, bs, c)
                attn_mask = attn_mask.repeat(fakesetsize, 1, 1)  # (num_head, 108, pixel_h*pixel_w)
                ...
                for qid in range(intermediate_querys.shape[0]):
                    if qid < 1:  # training mode sqr query-stage alignment
                        lvl = 0
                    ...
                    if lvl < self.num_layers - 1:
                        outputs_class, outputs_mask, attn_mask, _, _ = self.forward_prediction_heads(
                            intermediate_querys[qid], intermediate_pixels[qid], pixel_feature_size, lvl, return_attn_mask=True,
                        )
                    else:
                        outputs_class, outputs_mask, _, matching_indices, gt_attn_mask = self.forward_prediction_heads(
                            intermediate_querys[qid], intermediate_pixels[qid], pixel_feature_size, lvl,
